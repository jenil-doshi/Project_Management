		...
		try {
			...
			if (project.getStatus().equals(Constants.PROJECT_NEW)
				...
				if ((project.getOwner().getUserId() == userId && userRole.equals(Constants.ROLE_ADMIN))) {
					...
					attributes.addAttribute("error", "Error Adding Task. Either User is not the owner of project or user is not added to the project");
					return "redirect:/project/addTask/"+ projectId;
				}
			} else {
				attributes.addAttribute("error", "Task cannot be added. Project should be in New, Planning or Ongoing state");
				return "redirect:/project/addTask/"+ projectId;
			}
